...
class LittleFaeries(Npc):
    def __init__(self, name, side, mana, life, images, artifacts, quests, x, y, pos, groups,
        ...
        magical_crystal_image = pygame.image.load(os.path.join(path, "resources/graphics/artifacts", "magical_crystal.PNG")).convert_alpha()
        self.magical_crystal = Artifact(magical_crystal_image, 15, 'Magical Crystal', None)
        magic_dust_image = pygame.image.load(os.path.join(path, "resources/graphics/artifacts", "magic_dust.PNG")).convert_alpha()

        self.magic_dust = Artifact(magic_dust_image, 20, 'Magic Dust', None)
        self.gifts.add(self.magical_crystal, self.magic_dust)

        faerie_attack = pygame.image.load(os.path.join(path, "resources/graphics/particles", "spell.PNG")).convert_alpha()
